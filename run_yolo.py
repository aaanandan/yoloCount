...
def update_people_count(frame, detections):
    ...
    # Sort detections by x-coordinate for consistent numbering
    sorted_detections = sorted(detections["pred"], key=lambda det: det[0])
    ...
    for idx, det in enumerate(sorted_detections, 1):
        ...
        # Draw just the number above the person
        cv2.putText(frame, f"{idx}", (x1, y1 - 10), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
        
        if person_id not in people_tracking:
            people_tracking[person_id] = {"last_y": person_center_y, "counted": False}
        
        # Check if person touches or crosses the line
        if not people_tracking[person_id]["counted"]:
            # Check if the person's bounding box intersects with the line
            if (y1 <= DETECTION_LINE_Y <= y2):
                total_people_count += 1
                people_tracking[person_id]["counted"] = True
        
        people_tracking[person_id]["last_y"] = person_center_y
        current_frame_ids.add(person_id)
    
    # Clean up tracking dictionary
    people_tracking = {k: v for k, v in people_tracking.items() if k in current_frame_ids}
    
    # Add total count to frame (in top-left corner)
